...
# Directory paths
gpt_3_5_outputs_dir = "gpt_3.5_outputs"
metadata_dir = "metadata"

# File paths
metadata_file = os.path.join(metadata_dir, 'metadata.jsonl')
merged_file = os.path.join(metadata_dir, 'merged_gpt3.5.jsonl')
...
with open(merged_file, 'w', encoding='utf-8') as out_f:
    for filename in os.listdir(gpt_3_5_outputs_dir):
        if filename.endswith('.json'):
            file_path = os.path.join(gpt_3_5_outputs_dir, filename)
            ...
            try:
                ...
                if "vendor_name" in data["gpt_3.5_predictions"]:
                    data["gpt_3.5_predictions"]["vendor_name"] = data["gpt_3.5_predictions"]["vendor_name"].lower()
                ...
                if matches:
                    ...
                    if "vendor_name" in ground_truth:
                        ...
                        if ground_truth["vendor_name"] in data["gpt_3.5_predictions"]["vendor_name"]:
                            data["gpt_3.5_predictions"]["vendor_name"] = ground_truth["vendor_name"]
                    
                    # Merge the metadata with the prediction
                    matched_entry = {
                        "file_name": data["file_name"].replace('.txt', '.jpg'),
                        "ground_truth": json.dumps(ground_truth),
                        "predictions": json.dumps(data['gpt_3.5_predictions'])
                    }
